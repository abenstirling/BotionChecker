document.addEventListener('DOMContentLoaded', function() {
  ...
  function checkLinksOnPage() {
      ...
      chrome.runtime.sendMessage({action: "getPageContent"}, function(response) {
          if (response.error) {
              ...
              allLinks = response.checkedLinks.map(link => ({...link, foundOn: response.url}));
              displayLinks(allLinks, linkList);
  ...
  function displayLinks(links, container) {
      ...
      links.forEach(function(link) {
          ...
          } else if (link.status === 'Exists (Cross-origin)' || link.status === 'mailto') {
  ...
  function startAutomatedCrawl() {
      ...
      allLinks = []; // Reset allLinks before starting the crawl
  ...
  function updateProgress(progress) {
      ...
      // Accumulate links as they are crawled
      allLinks = allLinks.concat(progress.newLinks);
  }
  ...
  function displayReport(errors) {
      ...
      if (errors.length === 0) {
          ...
          errors.forEach(error => {
              ...
              const sourcePage = allLinks.find(link => link.url === error.url)?.foundOn || 'Unknown page';
              li.innerHTML = `<strong>${error.url}</strong> (Status: ${statusText})<br>Found on: ${sourcePage}`;
              li.classList.add('error');
              ul.appendChild(li);
          });
          reportDiv.appendChild(ul);
      }
      // Display all links after the crawl
      displayLinks(allLinks, linkList);
  }
